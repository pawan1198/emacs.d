...
;; Package-Version: 20230904.1837
;; Package-Revision: e47e85087944
...
If BUFFER is nil, use `ledger-import-buffer' instead."
  (with-current-buffer (or buffer (ledger-import-buffer))
    (when (and (functionp 'ledger-mode) (functionp 'ledger-mode-clean-buffer))
      (ledger-mode)
...
(defun ledger-import--current-ledger-file ()
  ...
  (when (and (buffer-file-name) (or (derived-mode-p 'ledger-mode) (derived-mode-p 'hledger-mode)))
    (buffer-file-name)))
      ...
      (make-process
       ...
       :sentinel (lambda (_process event)
                   (when (and callback (string= event "finished\n"))
                     (with-current-buffer (ledger-import-buffer)
                       (goto-char (point-min))
